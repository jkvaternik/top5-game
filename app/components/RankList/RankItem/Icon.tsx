const basicColorRainbowBg = ['bg-red-500', 'bg-orange-400', 'bg-yellow-400', 'bg-green-500', 'bg-blue-500']

export function NumberIcon({ number, isEmpty }: {
  number: number,
  isEmpty: boolean,
}): JSX.Element {
...
export function StringIcon({ string, isEmpty }: {
  string: string,
  isEmpty: boolean,
}): JSX.Element {
  const styles: React.CSSProperties = { position: 'absolute', width: '100%', height: '100%', WebkitBackfaceVisibility: 'hidden', backfaceVisibility: 'hidden' };
